...
```text
src/
  recon.cpp                       Recon compression prototype
...
- `build/recon` is compiled.
- `results/toy_graph.recon.bin` is generated.
...
```bash
build/recon INPUT_PATH OUTPUT_PATH PARTITION_LIMIT MIN_COMMON MAX_POSTINGS THREADS VERIFY DISABLE_RESIDUAL_STITCHING
```

Example:

```bash
build/recon data/toy/toy_graph.mtx results/toy_graph.recon.bin 64 2 4096 4 1 0
```
...
```bash
build/downstream_tasks_bench data/toy/toy_graph.mtx results/toy_graph.recon.bin 1
```
