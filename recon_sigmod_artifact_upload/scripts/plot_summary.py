...
def plot_compression():
    ...
    ax.set_ylabel("Recon chain / 2|E|")
    ax.set_title("Recon compression summary from processed results")
...
def plot_ablation():
    ...
    sg = [float(r["sg_over_recon"]) for r in rows]
    fb = [float(r["fb_over_recon"]) for r in rows]

    x = range(len(rows))
    width = 0.38
    fig, ax = plt.subplots(figsize=(9, 3.5))
    ax.bar([i - width / 2 for i in x], sg, width, label="SG-CN / Recon",
           color="white", edgecolor="#333333", hatch="///")
    ax.bar([i + width / 2 for i in x], fb, width, label="FB-CN / Recon",
           color="white", edgecolor="#333333", hatch="\\\\\\")
