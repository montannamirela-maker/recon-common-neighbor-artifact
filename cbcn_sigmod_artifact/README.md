...
This repository contains an anonymized research artifact for the paper
**CBCN: A Common-Neighbor-Based Executable Representation for Graph Analytics**.

The artifact includes the implementation of the CBCN compression prototype,
downstream benchmark code, scripts for a small sanity-check run, processed result
summaries used by the paper, and instructions for obtaining the public datasets.
It is intended to support reviewer inspection under double anonymity.

## Repository Layout

```text
src/
  cbcn.cpp                       CBCN compression prototype
...
The paper experiments were run on a Linux server with dual Intel Xeon E5-2696 v4
processors, 88 hardware threads, and 123 GB RAM. The toy example below should
run on a laptop.
...
- The output prints compression statistics and reports verification as `PASSED`.

## Running CBCN on a Graph

Input graphs can be provided as Matrix Market files or edge-list files. The
compressor accepts positional arguments:
...
author names, institution names, personal paths, and private URLs. See
`ANONYMITY.md` for the checklist used before upload.
